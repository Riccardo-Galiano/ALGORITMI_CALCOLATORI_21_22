...
bool Professor::setNoAvaibilities(int acYear, std::string &input) {
    ...
    if(_noAvailab.first != acYear) {//se non abbiamo ancora un anno accademico o è diverso da quello nuovo
        ...
        _noAvailab = (std::pair<int,std::vector<std::pair<Date,Date>>>(acYear,vect));//assegno la prima tupla di Date all'anno accademico
    }else

        _noAvailab.second.emplace_back(d1,d2);//_nonAvaib.second.push_back(std::pair(d1,d2))così non me la fa inserire!!
