...
std::string Professor::outputAvailabilities(int nMatr,int acYear) {


       auto iterNonAnvaib = _nonAvaib.find(acYear);
        std::stringstream output;
        int i = 0;

            output << iterNonAnvaib->first << "-" << iterNonAnvaib->first + 1 << ";" << "d" << setId(nMatr) << ";";
            for (auto iterDate = iterNonAnvaib->second.begin(); iterDate != iterNonAnvaib->second.end(); iterDate++) {
                ...
                if (i < iterNonAnvaib->second.size() - 1)
                    output << ";";
                i++;
            }
...
bool Professor::availabilityPeriodIsEmpty(int year) {
    auto iterator = _nonAvaib.find(year);
    return iterator->second.empty();
}

void Professor::availabilityClear() {
  _nonAvaib.clear();
}
